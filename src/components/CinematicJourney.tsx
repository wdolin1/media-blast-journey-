...
function drawPlaceholderFrame(ctx: CanvasRenderingContext2D, w: number, h: number, stage: Stage, alpha: number) {
  ...
  ctx.fillStyle = 'rgba(236,231,221,0.06)'
  ctx.font = `800 ${Math.round(h * 0.6)}px Archivo, sans-serif`
  ctx.textAlign = 'center'
  ctx.textBaseline = 'middle'
  ctx.fillText(stage.number, w / 2, h * 0.52)

  const inset = Math.round(Math.min(w, h) * 0.045)
  ctx.strokeStyle = 'rgba(236,231,221,0.18)'
  ...
  ctx.font = `600 ${Math.max(12, Math.round(h * 0.018))}px "IBM Plex Mono", monospace`
  ctx.fillText(`PLACEHOLDER FRAME · ${stage.name}`.toUpperCase(), w / 2, h * 0.7)
  ctx.restore()
}

export function CinematicJourney({ stages, brand, brandAccent, kicker, title, frames }: CinematicJourneyProps) {
  ...
  return (
    <section ref={trackRef} className="relative" style={{ height: `${trackHeightVh}vh` }}>
      <div ref={pinRef} className="sticky top-0 h-screen overflow-hidden bg-deep">
        <canvas ref={canvasRef} className="block h-full w-full" />
        <div
          className="pointer-events-none absolute inset-0"
          style={{
            background:
              'linear-gradient(180deg, rgba(6,7,10,.76) 0%, rgba(6,7,10,0) 30%, rgba(6,7,10,.08) 58%, rgba(6,7,10,.90) 100%)',
          }}
        />

        {/* masthead */}
        <div className="absolute inset-x-0 top-0 z-10 px-6 pt-8 md:px-[30px]">
          <div className="mx-auto flex max-w-[1180px] flex-wrap items-baseline gap-3.5">
            <div className="font-display text-xs font-extrabold tracking-[0.24em] text-[#F2EDE3] uppercase">
              {brand}
              <span className="text-amber">{brandAccent}</span>
            </div>
            <div className="font-mono text-[11.5px] tracking-[0.16em] text-[#F2EDE3]/60 uppercase">{kicker}</div>
          </div>
          <h1 className="mx-auto mt-4 max-w-[1180px] px-0 font-display text-[clamp(36px,6.4vw,82px)] leading-[0.93] font-extrabold tracking-[-0.025em] text-[#F7F3EA] [text-shadow:0_2px_34px_rgba(0,0,0,.55)]">
            {title}
          </h1>
        </div>
        ...
        <div className="absolute inset-x-0 bottom-0 z-10 px-6 pb-10 md:px-[30px]">
          ...
          >
            <div className="pt-2.5 font-mono text-xs tracking-[0.1em] text-amber">{stage.number}</div>
            <div>
              <h2 className="m-0 font-display text-[clamp(22px,3.2vw,34px)] leading-[1.1] font-bold tracking-[-0.012em] text-[#F7F3EA]">
                {stage.name}
              </h2>
              <p className="mt-2.5 max-w-[56ch] text-[17px] leading-[1.55] text-[#F2EDE3]/82">{stage.body}</p>
            </div>
          </div>
        </div>

        {frames && <div ref={bootRef} className="absolute bottom-0 left-0 z-20 h-0.5 w-0 bg-amber transition-[width] duration-200 ease-linear" />}
      </div>
    </section>
  )
}
